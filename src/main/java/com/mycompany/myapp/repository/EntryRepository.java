...
public interface EntryRepository extends JpaRepository<Entry, Long> {
    ...
    @Query(value = "select distinct entry from Entry entry left join fetch entry.keywords",
        countQuery = "select count(distinct entry) from Entry entry")
    Page<Entry> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct entry from Entry entry left join fetch entry.keywords")
    List<Entry> findAllWithEagerRelationships();

    @Query("select entry from Entry entry left join fetch entry.keywords where entry.id =:id")
